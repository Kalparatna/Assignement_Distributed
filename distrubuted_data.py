...
def validate_and_insert():
    ...
    users_data = [u for u in users_data if None not in u]
    products_data = [p for p in products_data if None not in p and p[2] > 0]  
    orders_data = [o for o in orders_data if None not in o and o[3] > 0 and o[2] <= len(products_data)] 
    ...
    for db_name, query, data in zip(['users', 'products', 'orders'], 
                                    ...
                                    [users_data, products_data, orders_data]):
